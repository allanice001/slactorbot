...
def _new_plugin(actor_system, plugin_actors, plugins_dir, plugin_name):
    plugin_import = importlib.import_module('slactorbot' + '.' + 'plugins' + '.' + plugin_name)
...
def _change_plugin(actor_system, plugin_actors, plugins_dir, plugin_name):
    print "changed plugin: %s" % plugin_name
    plugin_actors = _delete_plugin(actor_system, plugin_actors, plugin_name)
    plugin_actors = _new_plugin(actor_system, plugin_actors, plugins_dir, plugin_name)
    return plugin_actors
...
def load_plugins(actor_system, plugin_actors):
    plugins_dir = os.path.join('slactorbot', 'plugins')
        ...
        if _plugin.endswith('.py') and not _plugin.startswith('_'):
            ...
            try:
                ...
                if plugin_md5 != plugin_actors[plugin_name]['md5']:
                    plugin_actors = _change_plugin(actor_system, plugin_actors, plugins_dir, plugin_name)
